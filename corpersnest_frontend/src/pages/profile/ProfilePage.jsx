 ...
 *
 * Tabs: Account info | NYSC profile | My listings
 * - Edit profile inline
 * - NYSC section for state code, stream, camp date
 * - Role badge with correct colors per role
 */
...
function InfoRow({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between text-sm py-3 border-b last:border-0"
      style={{ borderColor: 'var(--border)' }}>
      <span className="flex items-center gap-2" style={{ color: 'var(--text-muted)' }}>
        {icon}{label}
      </span>
      <span className="font-medium text-right ml-4 truncate max-w-[180px]"
...
export default function ProfilePage() {
  ...
  const [nyscForm, setNyscForm] = useState({
    callup_number:   user?.callup_number   || '',
    nysc_state_code: user?.nysc_state_code || '',
    stream:          user?.stream?.toString() || '',
    camp_start_date: user?.camp_start_date || '',
  })

  const [lga, setLga] = useState(
    user?.landlord_profile?.lga || ''
  )
  ...
  const handleAvatarChange = async (file) => {
    ...
    try {
      ...
      fd.append('folder', 'corpersnest/avatars')
  ...
  const handleSaveNysc = async () => {
    const errs = {}
    if (nyscForm.callup_number && !/^NYSC\/[A-Z]{2,5}\/\d{4}\/\d+$/i.test(nyscForm.callup_number.trim()))
      errs.callup_number = 'Format: NYSC/XXX/YYYY/NNNNNN'
    if (nyscForm.nysc_state_code && !/^[A-Z]{2}\/\d{2}[ABC]\/\d+$/i.test(nyscForm.nysc_state_code.trim()))
      errs.nysc_state_code = 'Format: AB/25A/2008'
    if (nyscForm.stream && !['1', '2'].includes(nyscForm.stream))
      errs.stream = 'Must be 1 or 2'
    setNyscErrors(errs)
    ...
    try {
      ...
      if (isLandlord && lga !== (user?.landlord_profile?.lga || '')) {
        const updated = await authApi.updateLandlordProfile({ lga })
        updateUser(updated)
      }

      setEditingNysc(false)
      toast.success('NYSC profile updated')
    } catch (err) {
      toast.error(err.message)
    } finally {
      setSavingNysc(false)
        ...
        <div className="card p-5 mb-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="relative shrink-0">
              ...
              >
                {avatarLoading
                  ? <Spinner size="sm" />
                  : <span className="opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center w-full h-full rounded-full" style={{ background: 'rgba(0,0,0,0.45)' }}>
                      {Icon.camera}
                    </span>
                }
            ...
            <div className="flex-1">
              <h1 className="font-semibold text-lg leading-tight mb-1"
                style={{ color: 'var(--text-primary)' }}>
                {user.full_name}
              </h1>
            ...
            {isAdmin && (
              <Link to="/admin">
                <Button size="sm" variant="ghost">Admin panel</Button>
              </Link>
            )}
          </div>
          ...
          {user.status === 'suspended' && (
            <div className="p-3 rounded-xl border mb-3 text-xs"
              ...
              <p className="font-medium mb-0.5">Account suspended</p>
          ...
          {isLandlord && user.status === 'pending_verification' && (
            <div className="p-3 rounded-xl border mb-3 text-xs"
              ...
              <p className="font-medium mb-0.5">Pending verification</p>
              Our team is reviewing your account. You'll be able to post once approved.
            </div>
          )}

          {isPcm && !user.nysc_state_code && (
            <div className="p-3 rounded-xl border mb-3 text-xs"
              style={{ background: 'var(--brand-light)', borderColor: 'var(--brand)', color: 'var(--brand-dark)' }}>
              <p className="font-medium mb-0.5">Complete your NYSC profile</p>
              Add your state code to unlock full access.{' '}
              <button onClick={() => setActiveTab('nysc')}
          ...
          <div className="card p-5 flex flex-col gap-4 animate-fade-in">
            {!editingInfo ? (
              ...
              <div className="flex flex-col gap-4 animate-fade-in">
                <Input label="Full name" value={infoForm.full_name}
                  onChange={e => setInfoForm(p => ({ ...p, full_name: e.target.value }))} />
                <Input label="Phone number" type="tel" placeholder="08012345678"
                  value={infoForm.phone_no}
                  onChange={e => setInfoForm(p => ({ ...p, phone_no: e.target.value }))} />
                <div className="flex gap-2">
                  <Button fullWidth size="sm" loading={savingInfo} onClick={handleSaveInfo}>
                    Save changes
                  </Button>
            ...
            {!editingNysc ? (
              <div className="flex flex-col gap-1">
                {!isLandlord && (
                  <>
                    <InfoRow icon={
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>
                    } label="Callup number" value={user.callup_number || '—'} />
                    <InfoRow icon={
                    ...
                    } label="State code" value={user.nysc_state_code || '—'} />
                    <InfoRow icon={
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
                    } label="Stream" value={user.stream ? `Stream ${user.stream}` : '—'} />
                    <InfoRow icon={
                    ...
                    } label="Camp start" value={user.camp_start_date || '—'} />
                  </>
                )}
                {isLandlord && (
                  <InfoRow icon={
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>
                  } label="LGA" value={user.landlord_profile?.lga || '—'} />
                )}
              ...
              <div className="flex flex-col gap-4 animate-fade-in">
                {!isLandlord && (
                  <>
                    <Input label="Callup number" placeholder="NYSC/FUA/2025/107449"
                      value={nyscForm.callup_number}
                      onChange={e => setNyscForm(p => ({ ...p, callup_number: e.target.value }))}
                      error={nyscErrors.callup_number} />
                    <Input label="State code" placeholder="AB/25A/2008"
                      value={nyscForm.nysc_state_code}
                      onChange={e => setNyscForm(p => ({ ...p, nysc_state_code: e.target.value }))}
                      error={nyscErrors.nysc_state_code} />
                    <div>
                      <label className="text-sm font-medium block mb-1.5"
                        style={{ color: 'var(--text-primary)' }}>Stream</label>
                      <div className="flex gap-2">
                        {['1', '2'].map(s => (
                          <button key={s} type="button"
                            ...
                            style={{
                              background:  nyscForm.stream === s ? 'var(--brand)' : 'var(--bg-subtle)',
                              color:       nyscForm.stream === s ? 'white' : 'var(--text-secondary)',
                              borderColor: nyscForm.stream === s ? 'var(--brand)' : 'var(--border)',
                            }}>
                            Stream {s}
                          </button>
                        ))}
                    ...
                    <Input label="Camp start date" type="date"
                      value={nyscForm.camp_start_date}
                      onChange={e => setNyscForm(p => ({ ...p, camp_start_date: e.target.value }))} />
                  </>
                )}
                {isLandlord && (
                  <div>
                    <label className="text-sm font-medium block mb-1.5"
                      style={{ color: 'var(--text-primary)' }}>LGA</label>
                ...
                <div className="flex gap-2">
                  <Button fullWidth size="sm" loading={savingNysc} onClick={handleSaveNysc}>
                    Save
                  </Button>
                  <Button fullWidth size="sm" variant="ghost" onClick={() => setEditingNysc(false)}>
                    Cancel
                  </Button>
                </div>
              </div>
