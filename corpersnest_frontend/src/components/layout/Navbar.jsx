...
function NavIcon({ to, label, icon, badge, onClick }) {
...
function NotificationDropdown({ notifications, onMarkAllRead, onClose }) {
  ...
  return (
    <div className="absolute right-0 top-11 w-80 rounded-2xl shadow-xl border z-50 overflow-hidden"
      style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      <div className="flex items-center justify-between px-4 py-3 border-b"
        style={{ borderColor: 'var(--border)' }}>
        <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Notifications</p>
        <button onClick={onMarkAllRead} className="text-xs hover:underline"
          style={{ color: 'var(--brand)' }}>Mark all read</button>
      </div>
    ...
    <header className="sticky top-0 z-40 border-b"
      ...
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 ml-auto">
          {user && (
            <>
              <NavIcon to="/search" label="Search" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>} />
              <NavIcon to="/connections" label="Connections" badge={pendingConns}
                icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M19 8v6M22 11h-6"/></svg>}
              />
              <NavIcon to="/messages" label="Messages" badge={unreadMsgs}
                icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>}
              />

              {/* Bell — notifications */}
              <div className="relative" ref={dropdownRef}>
                <NavIcon to="/notifications" label="Notifications" badge={unreadNotifs}
                  onClick={handleBellClick}
                  icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/></svg>}
                />
                {showNotifs && (
          ...
          {/* Theme toggle */}
          <button onClick={toggle} className="w-9 h-9 rounded-xl flex items-center justify-center transition-all"
          ...
          {user ? (
            <div className="flex items-center gap-2">
              <Link to="/profile"><Avatar name={user.full_name} src={user.profile_picture_url} size="sm" /></Link>
              <button onClick={() => { logout(); navigate('/') }}
                className="hidden md:block text-xs px-2 py-1 rounded-lg transition-colors"
                style={{ color: 'var(--text-muted)' }}>Logout</button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <Link to="/login" className="text-sm font-medium px-3 py-2" style={{ color: 'var(--text-secondary)' }}>Login</Link>
              <Link to="/register" className="text-sm font-medium text-white px-4 py-2 rounded-xl" style={{ background: 'var(--brand)' }}>Join</Link>
            </div>
          )}
        </div>
      </div>
